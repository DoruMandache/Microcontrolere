   ...
   while(1) // bucla fără sfârşit
    {
      ...
      output_low(PIN_C5); // ledul conectat la pinul RC5 stins (trecerea în 0 logic)
      
      delay_ms(1000); // întârziere de 1000 ms
      
      output_low(PIN_C0); // ledul conectat la pinul RC0 stins (trecerea în 0 logic)
      output_high(PIN_C5); // aprinderea ledului conectat la pinul RC5 (trecerea în 1 logic)
